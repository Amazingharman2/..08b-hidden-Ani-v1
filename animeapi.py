...
def get_streaming_links(url_path):
    """
    Extracts streaming links from an episode page.
    """
    base_url = "https://animesalt.cc"
    if not url_path.startswith('episode/'):
        url_path = f"episode/{url_path}"
...
def api_documentation():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/api/stream/<path>": "Get streaming links for an episode",
            "/api/health": "Health check endpoint"
        },
        "examples": {
            "search": "/api/search?q=naruto",
            "anime_info": "/api/anime/series/naruto-shippuden",
            "streaming_links": "/api/stream/naruto-shippuden-1x1"
        }
    })
